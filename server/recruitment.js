...
function isDuplicateApplicationError(err) {
  ...
  return (
    code === "23505" ||
    constraint === "uq_job_apps_job_email" ||
    message.includes("uq_job_apps_job_email") ||
  ...
  try {
    ...
    if (req.method === "POST" && resource === "apply") {
      ...
      const duplicate = await sql`
        ...
        WHERE job_id = ${jobId} AND lower(email) = lower(${email})
        LIMIT 1
      `;
      if (duplicate.length) {
        return send(res, 409, { ok: false, error: "You already applied to this vacancy with this email" });
      }
